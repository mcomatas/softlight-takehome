...
Unhandled cases:
- The document tree has five different node types, these five being Document, Canvas, Frame, Rectangle, and Text. Figma has more node types than this, but for this assignment I only needed to handle the Frame, Rectangle, and Text nodes.
- When generating border / stroke I only handled the case when stroke type is SOLID. I know there can be different gradient borders, but as they did not show up in the design I did not code it for simplicity.
...
- The 'Sign in' and 'Create account' text on the frames on the bottoms of the page were tricky to get visually the same. The API gives a font weight of 600, but when generating CSS with that it looks a tad too bold. I believe this is because of the different way Figma is handling Semi-Bold font and the browser is handling font weight 600. I chose to edit down the weight of these two by adding the "font-variation-settings: 'wght' 520;" to them.